  ...
  <div class="px-4" data-no-month-swipe data-testid="historial-diario">
      ...
      <div v-for="i in 3" :key="i" class="space-y-1.5">
        ...
        <div v-if="i === 1" class="ml-4 space-y-1.5">
          <div
            v-for="j in 2"
            :key="j"
            class="bg-theme-card rounded-xl h-12 shimmer border border-theme-border opacity-70"
          ></div>
        </div>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div
        v-if="mostrarSemana"
        class="space-y-3"
        :class="{ 'mb-6': ambasVistas && vistaActiva === 'semana' }"
      >
        <div v-for="semana in gastosPorSemana" :key="semana.key">
          <button
            ...
            :class="
              semanaExpandida === semana.key
                ? 'bg-theme-accent-bg border border-theme-accent shadow-sm'
                : 'bg-theme-card border border-theme-border hover:bg-theme-card'
            "
            @click="toggleSemana(semana.key)"
          >
            <div class="text-left">
              <p class="text-sm font-medium text-theme-text">
                {{ formatRangoSemana(semana.desde, semana.hasta) }}
              </p>
              <p class="text-xs text-theme-text-sec">
                {{ semana.dias.length }} {{ semana.dias.length === 1 ? 'dia' : 'dias' }} con gastos
              </p>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-sm font-semibold text-theme-accent"
                >{{ currencySymbol }}&nbsp;{{ formatMonto(semana.total) }}</span
              >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-4 h-4 text-theme-text-sec transition-transform"
              ...
              <div v-for="dia in semana.dias" :key="dia.fecha">
                <button
                  ...
                  :class="
                    diasExpandidos.has(dia.fecha)
                      ? 'bg-theme-border-md border border-theme-border'
                      : 'bg-theme-card hover:bg-theme-card'
                  "
                ...
                >
                  <div class="flex items-center gap-2.5">
                    ...
                    >
                      <span class="text-[0.625rem] uppercase leading-none font-semibold">{{
                        nombreDiaSemana(dia.fecha)
                      }}</span>
                      <span class="text-xs font-bold leading-tight">{{
                        extraerDia(dia.fecha)
                      }}</span>
                      <span
                    ...
                    <div
                      v-else
                      class="w-9 h-9 rounded-lg flex flex-col items-center justify-center bg-theme-card border border-theme-border shrink-0"
                    >
                      <span
                        class="text-[0.625rem] uppercase leading-none font-semibold text-theme-text-muted"
                        >{{ nombreDiaSemana(dia.fecha) }}</span
                      >
                      ...
                      <div class="flex items-center justify-between gap-2">
                        ...
                        <p class="text-xs font-medium text-theme-text-sec truncate">
                          ...
                          <span class="text-theme-text-muted font-normal"
                            >· {{ formatRelativo(dia.fecha) }}</span
                          >
                        </p>
                        <span class="text-sm font-semibold text-theme-text shrink-0"
                          >{{ currencySymbol }}&nbsp;{{ formatMonto(dia.total) }}</span
                        >
                      </div>
                      <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        <span class="text-[0.6875rem] text-theme-text-muted leading-none"
                          >{{ dia.gastos.length }}
                          {{ dia.gastos.length === 1 ? 'gasto' : 'gastos' }}</span
                        >
                        <span
                          v-if="getStats(dia).topCat"
                          class="text-[0.6875rem] px-1 py-0.5 rounded leading-none font-medium"
                          :style="{
                            backgroundColor: getStats(dia).topCat.color + '20',
                            color: getStats(dia).topCat.color,
                          }"
                        >
                          {{ getStats(dia).topCat.icono }} {{ getStats(dia).topCat.nombre }}
                        </span>
                        <span
                          v-if="getStats(dia).rangoHoras"
                          class="text-[0.6875rem] text-theme-text-muted leading-none"
                          >· {{ getStats(dia).rangoHoras }}</span
                        >
                      </div>
                    </div>
                    <div class="flex items-center gap-1 shrink-0">
                      ...
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="w-3.5 h-3.5 text-theme-text-muted transition-transform shrink-0"
                ...
                <Transition name="expand">
                  <TransitionGroup
                    v-if="diasExpandidos.has(dia.fecha)"
                    name="gasto-in"
                    tag="div"
                    class="ml-6 mt-1 space-y-1.5"
                  >
                    <RegistroGastoItem
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <span
                ...
                @click.stop="toggleSelectDia(dia)"
              >
                <span class="text-[0.6875rem] uppercase leading-none font-semibold">{{
                  nombreDiaSemana(dia.fecha)
                }}</span>
              ...
              <div
                ...
                :class="
                  diasExpandidos.has(dia.fecha)
                    ? 'bg-theme-accent-bg border border-theme-accent/30'
                    : 'bg-theme-border-md'
                "
              >
                <span
                  class="text-[0.6875rem] uppercase leading-none font-semibold"
                  :class="
                    diasExpandidos.has(dia.fecha) ? 'text-theme-accent' : 'text-theme-text-muted'
                  "
                  >{{ nombreDiaSemana(dia.fecha) }}</span
                >
                <span
                  class="text-base font-bold leading-tight"
                  :class="diasExpandidos.has(dia.fecha) ? 'text-theme-accent' : 'text-theme-text'"
                  >{{ extraerDia(dia.fecha) }}</span
                >
              </div>
              <div class="flex-1 min-w-0 text-left">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <span class="text-sm font-bold text-theme-text shrink-0"
                    >{{ currencySymbol }}&nbsp;{{ formatMonto(dia.total) }}</span
                  >
                </div>
                <div class="flex items-center gap-1.5 mt-1 flex-wrap">
                  <span
                    class="text-[0.6875rem] px-1.5 py-0.5 rounded-md bg-theme-border-md text-theme-text-sec leading-none font-medium"
                  >
                    {{ dia.gastos.length }} {{ dia.gastos.length === 1 ? 'gasto' : 'gastos' }}
                  </span>
                  <span
                    v-if="getStats(dia).topCat"
                    class="text-[0.6875rem] px-1.5 py-0.5 rounded-md leading-none font-medium"
                    :style="{
                      backgroundColor: getStats(dia).topCat.color + '20',
                      color: getStats(dia).topCat.color,
                    }"
    ...
    <Transition name="selection-bar">
      ...
      >
        ...
        <div class="flex flex-col min-w-0 shrink">
          ...
          <span class="text-xs font-bold text-theme-text leading-tight"
            >{{ selectedIds.size }} gasto{{ selectedIds.size === 1 ? '' : 's' }}</span
          >
        </div>
        <button
...
const sinDatos = computed(
  () => props.gastosPorDia.length === 0 && props.gastosPorSemana.length === 0,
)
