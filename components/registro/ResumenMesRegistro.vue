<template>
  ...
  >
      ...
      <div class="flex flex-col items-end justify-start text-right shrink-0 pt-0.5">
        ...
        <div v-if="presupuesto > 0" class="flex items-center gap-1.5 mt-0.5">
          <span
            ...
            :class="
              excedido ? 'text-red-400' : porcentaje > 70 ? 'text-amber-400' : 'text-emerald-400'
            "
          >
    ...
    <Transition name="cat-expand">
      <div
        v-if="detallesAbierto"
        class="relative mt-3 pt-3 border-t border-theme-border/40 space-y-3 overflow-hidden"
      >
        <!-- Hoy + acciones -->
        <div class="flex items-center justify-between gap-2">
          <div class="flex items-baseline gap-2 min-w-0">
            <span class="text-[0.6rem] text-theme-text-sec uppercase tracking-wider font-semibold"
              >Hoy</span
            >
            <span class="text-base font-bold text-theme-text tabular-nums">{{
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <p
              class="text-[0.65rem] uppercase tracking-wider font-semibold text-theme-text-sec mb-0.5"
            >
              Ritmo de gasto
            </p>
            <p class="text-xs text-theme-text leading-snug">
              A este ritmo gastarias
              <span class="font-bold tabular-nums"
                >{{ currencySymbol }}&nbsp;{{ formatMonto(proyeccion) }}</span
              >
              este mes.
            </p>
        ...
        <div v-if="categoriasResumen.length > 0">
          <p
            class="text-[0.65rem] uppercase tracking-wider font-semibold text-theme-text-sec mb-1.5 px-1"
          >
            Por categoria
          </p>
          <ul class="space-y-0.5">
            ...
            >
              <span class="text-base shrink-0" :style="{ color: cat.color }">{{
                cat.icono || '●'
              }}</span>
