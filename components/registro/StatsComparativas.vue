<template>
  <div class="px-4 py-2">
    ...
    <div v-if="vista === 'comparar'">
          ...
          :class="
            ins.tono === 'positivo'
              ...
              : ins.tono === 'negativo'
                ...
                : 'bg-theme-card border-theme-border text-theme-text-sec'
          "
        >
      ...
      <div class="mb-4">
        <p class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider mb-2">
          Comparar con:
        </p>
        <!-- Carrusel con pr-4: a 380px con texto grande los chips se cortaban al borde -->
        <div
          class="flex items-center gap-1.5 overflow-x-auto pb-1 pr-8 scrollbar-hide scroll-fade-r"
        >
          <button
          ...
          <select
            ...
            :class="
              mesComparEsAntiguo ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40' : ''
            "
            :value="mesComparEsAntiguo ? mesComparar : ''"
            @change="onSelectMesAntiguo($event)"
          >
            <option value="" disabled>Mes</option>
            <option v-for="m in mesesAntiguos" :key="m.key" :value="m.key">
              {{ m.label }}
            </option>
          </select>
        </div>
      ...
      <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="bg-theme-card rounded-xl p-3 border border-theme-border">
          ...
          <p class="text-lg font-bold text-theme-text">
            {{ currencySymbol }}&nbsp;{{ formatMonto(totalActual) }}
          </p>
          <p class="text-[0.6875rem] text-theme-text-sec mt-0.5">{{ mesActualLabel }}</p>
        </div>
        <div class="bg-theme-card rounded-xl p-3 border border-theme-border">
          <p class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider mb-1">
            Mes comparado
          </p>
      ...
      <div v-else>
        <p
          class="text-[0.6875rem] text-theme-text-muted uppercase tracking-wider font-semibold mb-2"
        >
          Por categoría
        </p>
        <div class="space-y-2">
          ...
          >
            ...
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                <span class="text-[0.6875rem] text-theme-text-muted w-14 shrink-0 truncate">{{
                  mesActualLabel.split(' ')[0]
                }}</span>
                <div class="flex-1 h-1.5 bg-theme-input rounded-full overflow-hidden">
                  <div
                    class="h-full rounded-full transition-all duration-500"
                    :style="{ width: cat.barActual + '%', backgroundColor: cat.color }"
                  ></div>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-[0.6875rem] text-theme-text-muted w-14 shrink-0 truncate">{{
                  mesCompararLabel.split(' ')[0]
                }}</span>
                <div class="flex-1 h-1.5 bg-theme-input rounded-full overflow-hidden">
                  <div
                    class="h-full rounded-full bg-gray-600 transition-all duration-500"
                    :style="{ width: cat.barComparar + '%' }"
                  ></div>
                </div>
                <span class="text-[0.6875rem] text-theme-text-muted shrink-0"
                  >{{ currencySymbol }}&nbsp;{{ formatMonto(cat.comparar) }}</span
                >
              </div>
            </div>
          </div>
            ...
            <div class="flex items-center justify-between mb-1.5">
              <div class="flex items-center gap-2">
                <span
                  ...
                  :class="m.esActual ? 'text-theme-accent' : 'text-theme-text-muted'"
                >
                  {{ m.label }}
                </span>
                <span
                  v-if="m.esActual"
                  class="text-[0.6875rem] bg-theme-accent-bg text-theme-accent border border-theme-accent px-1.5 py-0.5 rounded-full"
                  >actual</span
                >
              </div>
              <div class="flex items-center gap-2">
                ...
                >
                  ...
                  {{ Math.abs(m.variacion).toFixed(0) }}%
                </span>
                <span class="text-xs font-semibold text-theme-text"
                  >{{ currencySymbol }}&nbsp;{{ formatMonto(m.total) }}</span
                >
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div class="bg-theme-card rounded-xl p-3 border border-theme-border text-center">
              ...
              <p class="text-sm font-bold text-red-400">
                {{ currencySymbol }}&nbsp;{{ formatMonto(maxTendencia) }}
              </p>
            </div>
            <div class="bg-theme-card rounded-xl p-3 border border-theme-border text-center">
              <p class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider mb-1">
              ...
              <p class="text-sm font-bold text-emerald-400">
                {{ currencySymbol }}&nbsp;{{ formatMonto(minTendencia) }}
              </p>
            </div>
          </div>
          <div
            class="bg-theme-card rounded-xl p-3 border border-theme-border flex items-center justify-between"
          >
            <p class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider">Promedio</p>
            <p class="text-base font-bold text-theme-text">
              {{ currencySymbol }}&nbsp;{{ formatMonto(promedioTendencia) }}
            </p>
          </div>
        </div>
      </div>
...
async function fetchGastosComparar(mes, anio) {
  ...
  try {
    ...
    isLoadingComparar.value = false
  }
}

const mesActualLabel = computed(() => `${MESES[props.mesActual - 1]} ${props.anioActual}`)
const mesCompararLabel = computed(() =>
  mesCompararObj.value
