<template>
  <div
    class="relative overflow-hidden rounded-2xl border border-theme-border bg-gradient-to-br from-theme-card to-theme-card/90 px-3 pt-2.5 pb-3.5 space-y-3"
  >
    ...
    <template v-if="resumenFuturos.totalProyectos > 0">
      ...
      <div class="relative text-center">
        <p class="text-[0.6875rem] uppercase tracking-[0.22em] text-theme-text-muted">
          Inversión estimada
        </p>
        ...
        <p class="mt-0.5 text-[0.6875rem] text-theme-text-sec">
          promedio de {{ resumenFuturos.totalProyectos }} proyecto{{
            resumenFuturos.totalProyectos > 1 ? 's' : ''
          }}
        </p>
      </div>
      ...
      <div class="relative">
        <div class="flex items-center justify-between text-[0.6875rem] mb-1.5">
          <span class="font-medium text-emerald-400"
            >min {{ currencySymbol }}&nbsp;{{ formatMonto(resumenFuturos.totalMinimo) }}</span
          >
          <span class="font-medium text-amber-300"
            >máx {{ currencySymbol }}&nbsp;{{ formatMonto(resumenFuturos.totalMaximo) }}</span
          >
        </div>
        <div
          class="relative h-1.5 w-full overflow-visible rounded-full bg-gradient-to-r from-emerald-500/70 via-sky-400/70 to-amber-400/70"
        >
          <div
      ...
      <div class="grid grid-cols-3 gap-2">
        ...
        <div class="rounded-xl border border-theme-border bg-theme-input p-2 text-center min-w-0">
          <p class="text-[0.6875rem] uppercase tracking-[0.18em] text-theme-text-muted">
            Decididos
          </p>
          <p class="mt-0.5 text-lg font-bold text-theme-text">
            {{ resumenFuturos.progresoDecision?.decididos || 0
            }}<span class="text-theme-text-muted font-semibold"
              >/{{ resumenFuturos.progresoDecision?.total || 0 }}</span
            >
          </p>
        </div>
      </div>
      ...
      <div v-if="resumenFuturos.progresoDecision?.total > 0" class="flex items-center gap-3">
        <div class="flex-1 h-2 overflow-hidden rounded-full bg-theme-input">
          <div
            ...
            :class="
              resumenFuturos.progresoDecision.porcentaje === 100
        ...
        <span
          ...
          :class="
            resumenFuturos.progresoDecision.porcentaje === 100 ? 'text-emerald-400' : 'text-sky-300'
          "
        >
      ...
      <div class="flex items-center justify-between gap-2 flex-wrap">
        <div class="flex items-center gap-3 text-[0.6875rem]">
          <div class="flex items-center gap-1.5">
            ...
            <span :class="desglose.compradas > 0 ? 'text-emerald-400' : 'text-theme-text-muted'"
              >{{ desglose.compradas }} compr.</span
            >
          </div>
          <div class="flex items-center gap-1.5">
            <span class="h-2 w-2 rounded-full bg-sky-400"></span>
            <span :class="desglose.planificadas > 0 ? 'text-sky-300' : 'text-theme-text-muted'"
              >{{ desglose.planificadas }} planif.</span
            >
          </div>
          <div class="flex items-center gap-1.5">
    ...
    <div
      v-else
      class="relative rounded-xl border border-dashed border-theme-border bg-theme-input/70 px-4 py-5 text-center"
    >
      <p class="text-sm text-theme-text">Aún no tienes proyectos futuros</p>
      <p class="mt-1 text-xs text-theme-text-sec">
        Crea uno para comparar alternativas como una PC nueva, ropa o equipamiento.
      </p>
    </div>
  </div>
</template>
