<template>
  ...
  >
    ...
    >
      ...
      <svg class="w-4 h-4 shrink-0 transition-transform" :class="abierto ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="abierto" class="px-2 pb-2 space-y-1">
      <button
        ...
        :class="!viendoPerfil ? 'bg-theme-accent/20 text-theme-accent font-semibold' : 'bg-theme-input text-theme-text-sec'"
        @click="elegir(null)"
      >
      ...
      <button
        ...
        type="button"
        class="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs text-left"
        :class="perfilActivoId === p.id ? 'bg-amber-500/25 text-amber-100 font-semibold' : 'bg-theme-input text-theme-text-sec'"
        @click="elegir(p.id)"
      >
...
<script setup>
const {
  perfiles, perfilActivoId, viendoPerfil, nombrePerfilActivo,
  fetchPerfiles, entrarPerfil,
} = usePerfiles()
