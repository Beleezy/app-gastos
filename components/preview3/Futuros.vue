<template>
  <div class="px-4 pt-3 pb-32">
    <Preview3PageHeader icon="🛍️" title="Gastos futuros" subtitle="Deseos y decisiones pendientes" accent="violet" />
    ...
    <template v-else>
      ...
      <template v-else>
        ...
        <div class="rounded-3xl border border-theme-border bg-gradient-to-br from-violet-500/5 to-theme-card p-4 mb-3">
          ...
          <Preview3Money :value="totalPromedio" tone="violet" class="text-[1.7rem] font-extrabold block mt-1" />
          <div class="flex items-center justify-between gap-2 mt-1.5 text-[0.7rem] text-theme-text-muted">
            ...
            <span class="shrink-0">
              rango <Preview3Money :value="totalMinimo" entero /> – <Preview3Money :value="totalMaximo" entero />
            </span>
          </div>
        </div>
            ...
            <button class="w-full text-left p-4 active:bg-theme-input/40 transition-colors" @click="toggle(p.id)">
              ...
              <div class="grid grid-cols-3 mt-3 rounded-xl bg-theme-input overflow-hidden">
                <div class="px-1.5 py-2 text-center min-w-0">
                  ...
                  <Preview3Money :value="p.resumen?.totalMinimo || 0" compact entero tone="green" class="text-[0.82rem] font-bold block mt-0.5" />
                </div>
                <div class="px-1.5 py-2 text-center border-x border-theme-border/60 min-w-0">
                  ...
                  <Preview3Money :value="p.resumen?.totalPromedio || 0" compact entero tone="sky" class="text-[0.82rem] font-bold block mt-0.5" />
                </div>
                <div class="px-1.5 py-2 text-center min-w-0">
                  <p class="text-[0.6rem] uppercase tracking-wide text-theme-text-muted">Máx</p>
                  <Preview3Money :value="p.resumen?.totalMaximo || 0" compact entero tone="amber" class="text-[0.82rem] font-bold block mt-0.5" />
                </div>
              </div>
                ...
                <div v-if="(det.opciones || []).length" class="mt-2 space-y-1.5">
                  <div v-for="op in det.opciones" :key="op.id" class="rounded-lg bg-theme-input/70 px-2.5 py-2">
                    ...
                    <p class="text-[0.68rem] text-theme-text-muted mt-0.5">
                      <Preview3Money :value="op.precioMinimo ?? op.precioPromedio ?? 0" compact entero tone="green" />
                      <span v-if="op.precioMaximo != null"> – <Preview3Money :value="op.precioMaximo" compact entero tone="amber" /></span>
                      <span v-if="op.precioPromedio != null" class="text-sky-400"> · prom. <Preview3Money :value="op.precioPromedio" compact entero /></span>
                    </p>
                  </div>
                </div>
