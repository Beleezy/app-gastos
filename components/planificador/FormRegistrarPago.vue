<template>
  <SharedBaseBottomSheet
    :title="gasto.gastoRegistradoId ? 'Editar registro de gasto' : 'Registrar pago'"
    @close="$emit('close')"
  >
    ...
    <div class="rounded-2xl border border-theme-border bg-theme-input p-4">
      <p class="text-sm font-semibold text-theme-text leading-snug break-words">
        {{ gasto.concepto }}
      </p>
      <div class="mt-1.5 flex items-center justify-between gap-3">
        <p class="min-w-0 truncate text-xs text-theme-text-sec">
          {{ gasto.categoriaNombre || 'Sin categoria' }} ·
          {{ formatFecha(gasto.fechaProbablePago) }}
        </p>
        <p class="shrink-0 text-sm font-semibold text-theme-text">
    ...
    <div>
      <label class="mb-1.5 block text-sm font-medium text-theme-text-muted"
        >Nota <span class="text-theme-text-muted">(opcional)</span></label
      >
      <textarea
        v-model="form.notas"
        rows="3"
