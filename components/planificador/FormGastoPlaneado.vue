<template>
  <SharedBaseBottomSheet
    :title="modoEdicion ? 'Editar gasto planificado' : 'Nuevo gasto planificado'"
    @close="$emit('close')"
  >
    <!-- Badge recurrente en edición -->
    <div
      v-if="modoEdicion && gastoEditar?.esRecurrente"
      class="flex items-start gap-2 rounded-xl border border-theme-accent/30 bg-theme-accent-bg px-3 py-2"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="w-4 h-4 text-theme-accent shrink-0 mt-0.5"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
      ...
      <div class="text-[0.6875rem] text-theme-accent leading-tight">
        Gasto recurrente. Al guardar elegirás si los cambios se aplican solo a este mes o también a
        los meses futuros.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-3 gap-2">
        ...
        >
          ...
          <span
            class="text-[0.6875rem] text-theme-text-muted text-center leading-tight line-clamp-2 break-words hyphens-none w-full"
            >{{ cat.nombre }}</span
          >
        </button>
      </div>
    ...
    <div
      v-if="esCategoriaAhorro"
      class="flex items-start gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="flex-1 min-w-0">
        ...
        <NuxtLink
          to="/ahorros"
          class="text-[0.6875rem] text-emerald-400 font-medium underline mt-1 inline-block"
        >
          Ver mis ahorros
        </NuxtLink>
      </div>
      ...
      <div class="bg-theme-input rounded-xl p-3 border border-theme-border">
        <div class="grid grid-cols-7 gap-1.5">
          <span
            ...
            :key="d"
            class="text-center text-[0.6875rem] text-theme-text-muted font-medium pb-1"
          >
            {{ d }}
          </span>
    ...
    <div class="flex items-center justify-between py-2">
      ...
      >
        <div
          ...
          :class="
            form.esRecurrente ? 'translate-x-[22px] bg-theme-card' : 'translate-x-0.5 bg-gray-400'
          "
        ></div>
      </button>
    </div>
    ...
    <div v-if="mostrarAlcance" class="fixed inset-0 z-[60] flex items-center justify-center px-6">
      ...
      >
        ...
        <div class="space-y-2">
          ...
          >
            Solo este mes
            <span class="block text-[0.6875rem] text-theme-text-muted mt-0.5"
              >Este registro se desvincula y no afecta los futuros</span
            >
          </button>
          <button
          ...
          >
            ...
            <span class="block text-[0.6875rem] text-theme-on-accent opacity-70 mt-0.5"
              >Propaga los cambios a todas las instancias del grupo</span
            >
          </button>
          <button
