<template>
  <div class="px-4 lg:px-0 pt-0 lg:pt-0 pb-2">
    ...
    <!-- Budget Summary Card -->
    <div class="relative bg-gradient-to-br from-theme-card to-theme-card/90 rounded-2xl px-4 pt-4 pb-4 space-y-4 border border-theme-border overflow-hidden">
      <!-- Decorative accent -->
        ...
        <div class="flex items-end justify-between">
          ...
          <div v-else class="flex items-baseline gap-0.5">
            <button class="flex items-baseline gap-0.5 hover:text-theme-accent-light transition-colors" @click="iniciarEdicion">
              <span class="text-lg font-bold text-theme-text-muted leading-none">{{ currencySymbol }}</span>
              <span class="text-3xl font-extrabold text-theme-text tracking-tight leading-none">{{ formatParteEntera(resumen.presupuesto) }}</span>
              <span class="text-base font-bold text-theme-text-muted leading-none">.{{ formatParteDecimal(resumen.presupuesto) }}</span>
            </button>
          </div>
      ...
      <!-- Stats Row: Planificado / Gastado / Disponible -->
      <div class="relative grid grid-cols-3 gap-2">
        <!-- Planificado -->
        <div class="bg-theme-input rounded-xl p-2.5 border border-orange-500/10 min-w-0">
          <div class="flex items-center gap-1.5 mb-1.5">
            <span class="w-2 h-2 rounded-full bg-orange-400"></span>
            <span class="text-[9px] text-theme-text-sec uppercase tracking-wider font-medium leading-none">Planificado</span>
          </div>
          <p class="text-[15px] font-bold text-orange-400 truncate leading-tight">{{ formatMonto(resumen.totalPlanificado) }}</p>
          <p class="text-[9px] text-theme-text-muted mt-0.5 leading-tight">del detalle</p>
        </div>
        <!-- Gastado -->
        <div class="bg-theme-input rounded-xl p-2.5 border min-w-0" :class="resumen.excedeGastoReal ? 'border-red-500/10' : 'border-emerald-500/10'">
          <div class="flex items-center gap-1.5 mb-1.5">
          ...
          <p class="text-[9px] text-theme-text-muted mt-0.5 leading-tight">{{ resumen.porcentajeGastadoReal.toFixed(0) }}% del plan</p>
        </div>
        <!-- Disponible -->
        <div class="bg-theme-input rounded-xl p-2.5 border min-w-0" :class="resumen.saldoReal >= 0 ? 'border-emerald-500/10' : 'border-red-500/10'">
          <div class="flex items-center gap-1.5 mb-1.5">
          ...
          <p v-if="analitica.esMesActual" class="text-[9px] text-theme-text-muted mt-0.5 leading-tight">{{ analitica.diasRestantes }} días rest.</p>
        </div>
      </div>
      ...
      <!-- Delta vs mes anterior + Ver análisis -->
      <div v-if="analitica.deltaPct !== null" class="flex items-center justify-between">
        ...
        <button
          ...
          @click="showAnalisis = !showAnalisis"
        >
          Ver análisis
          <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 transition-transform" :class="showAnalisis ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M5 15l7-7 7 7" />
          </svg>
        </button>
      </div>

      <!-- Analítica expandible: proyección, ritmo -->
      <Transition name="slide-down">
        <div v-if="analitica.esMesActual && resumen.presupuesto > 0 && showAnalisis" class="relative space-y-2">
          ...
          <div class="grid grid-cols-2 gap-2">
            <div class="bg-theme-input rounded-xl p-3">
              ...
              <p class="text-[9px] text-theme-text-muted leading-tight mt-0.5">
                por {{ analitica.diasRestantes }} día{{ analitica.diasRestantes === 1 ? '' : 's' }} rest.
              </p>
            </div>
            <div class="bg-theme-input rounded-xl p-3">
      ...
      <!-- Action buttons: Copiar mes + Exportar Excel -->
