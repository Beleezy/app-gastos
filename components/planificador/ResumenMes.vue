<template>
  <div class="px-4 lg:px-0 pt-0 lg:pt-0 pb-2" data-testid="resumen-mes">
    ...
    <!-- Budget Summary Card -->
    <div
      class="relative bg-gradient-to-br from-theme-card to-theme-card/90 rounded-2xl px-4 pt-4 pb-4 border border-theme-border overflow-hidden"
    >
      <!-- Decorative accent -->
      <div
        class="absolute top-0 right-0 w-32 h-32 bg-[var(--color-accent)]/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl"
      ></div>

      <!-- PRESUPUESTO MENSUAL label + big amount + Editar -->
      <div class="relative mt-1">
        <p
          class="text-[0.6875rem] text-theme-text-sec uppercase tracking-widest font-semibold mb-1"
        >
          Presupuesto mensual
        </p>
        <div class="flex items-end justify-between">
          ...
          <div v-else class="flex items-baseline gap-0.5">
            <button
              class="flex items-baseline gap-0.5 hover:text-theme-accent-light transition-colors"
              @click="iniciarEdicion"
            >
              <span class="text-2xl font-bold text-theme-text-muted">{{ currencySymbol }}</span>
              <span
                class="text-3xl font-extrabold text-theme-text tracking-tight"
                data-testid="monto-presupuesto"
              >
                {{ formatMonto(resumen.presupuesto) }}
              </span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-4 h-4 ml-1 text-theme-text-muted/50 hover:text-theme-accent transition-colors"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="2"
      ...
      <div class="relative grid grid-cols-2 gap-2 mt-4">
        <div
          class="bg-theme-input rounded-xl px-2.5 py-1.5 border min-w-0 flex flex-col justify-between"
          :class="resumen.excedeGastoReal ? 'border-red-500/10' : 'border-emerald-500/10'"
        >
          <div class="flex items-center gap-1.5 mb-1.5">
            <span
              class="w-2 h-2 rounded-full"
              :class="resumen.excedeGastoReal ? 'bg-red-400' : 'bg-emerald-400'"
            ></span>
            <span
              class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider font-medium leading-none"
              >Gastado</span
            >
          </div>
          <p
            class="text-[0.9375rem] font-bold truncate leading-tight"
            :class="resumen.excedeGastoReal ? 'text-red-400' : 'text-emerald-400'"
          >
        ...
        <div
          class="bg-theme-input rounded-xl px-2.5 py-1.5 border min-w-0 flex flex-col justify-between"
          :class="resumen.saldoReal >= 0 ? 'border-emerald-500/10' : 'border-red-500/10'"
        >
          <div class="flex items-center gap-1.5 mb-1.5">
            <span
              class="w-2 h-2 rounded-full"
              :class="resumen.saldoReal >= 0 ? 'bg-blue-400' : 'bg-red-400'"
            ></span>
            <span
              class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider font-medium leading-none"
              >Disponible</span
          ...
          <p
            class="text-[0.9375rem] font-bold truncate leading-tight"
            :class="resumen.saldoReal >= 0 ? 'text-white' : 'text-red-400'"
      ...
      <div
        class="flex items-center justify-between bg-theme-input rounded-xl px-3 py-1.5 mt-2 border border-orange-500/10"
      >
        <div class="flex items-center gap-1.5">
          <span class="w-2 h-2 rounded-full bg-orange-400"></span>
          <span class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider font-medium"
            >Planificado</span
          >
        </div>
      ...
      <div class="flex items-center justify-between mt-4">
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-1.5 bg-emerald-500/8 px-2.5 py-1.5 rounded-lg">
            ...
            <span class="text-[0.6875rem] text-emerald-400 font-bold ml-0.5">{{
              resumen.countPagados
            }}</span>
          </div>
          <div class="flex items-center gap-1.5 bg-orange-500/8 px-2.5 py-1.5 rounded-lg">
            ...
            <span class="text-[0.6875rem] text-orange-400 font-bold ml-0.5">{{
              resumen.countPendientes
            }}</span>
          </div>
        </div>
        <button
          ...
          <div class="space-y-2">
            <div>
              <div class="flex items-center justify-between text-[0.6875rem] font-medium mb-1">
                <span class="text-theme-text-sec uppercase tracking-wider"
                  >Planificado / Presupuesto</span
                >
                <span
                  :class="
                    resumen.porcentajeAsignado > 90 ? 'text-orange-400' : 'text-theme-text-muted'
                  "
                >
              ...
              <div class="relative w-full h-2 bg-theme-input rounded-full overflow-hidden flex">
                <div
                  ...
                  :class="
                    resumen.porcentajeAsignado > 90
                      ? 'bg-gradient-to-r from-orange-500 to-orange-400'
                      : 'bg-gradient-to-r from-[var(--color-accent-dark)] via-[var(--color-accent)] to-indigo-400'
                  "
          ...
          <div v-if="analitica.esMesActual && resumen.presupuesto > 0" class="space-y-2">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <p class="text-[0.6875rem] font-semibold text-red-400">
                  Proyección excede presupuesto
                </p>
                <p class="text-[0.6875rem] text-theme-text-muted leading-tight mt-0.5">
                  Al ritmo actual terminarías en {{ currencySymbol }}&nbsp;{{
                    formatMonto(analitica.proyeccionFinMes)
                  }}
                  ({{ currencySymbol }}&nbsp;{{ formatMonto(analitica.excesoProyectado) }} sobre
                  presupuesto)
                </p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <div class="bg-theme-input rounded-xl p-3">
                <p
                  class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider font-medium"
                >
                  Ritmo por día
                </p>
                <p class="text-base font-bold text-theme-text mt-1">
                  {{ currencySymbol }}&nbsp;{{ formatMonto(analitica.ritmoDiarioRecomendado) }}
                </p>
                <p class="text-[0.6875rem] text-theme-text-muted leading-tight mt-0.5">
                  por {{ analitica.diasRestantes }} día{{
                    analitica.diasRestantes === 1 ? '' : 's'
                  }}
                  rest.
                </p>
              </div>
            ...
            >
              ...
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2H7a2 2 0 00-2 2v2m4-6h6"
                />
              </svg>
              Plantillas
            </button>
