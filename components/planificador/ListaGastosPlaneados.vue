    ...
    <div class="mb-3">
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="
            mostrarFiltros || filtroActual !== 'todos'
        ...
        <div class="relative flex-1">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-theme-text-sec"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2"
      ...
      <div v-for="cat in categoriasFiltered" :key="cat.categoriaId" class="mb-5">
        ...
        <div class="mb-2.5">
          ...
          >
            ...
            ></span>
            <h3 class="text-xs font-semibold text-theme-text-muted uppercase tracking-wider flex-1">
              {{ cat.nombre }}
            </h3>
            <span class="text-xs text-theme-text-sec"
              >{{ cat.gastos.length }} ítem{{ cat.gastos.length !== 1 ? 's' : '' }}</span
            >
            <span class="text-xs font-semibold text-theme-text-sec"
          ...
          <div
            v-if="cat.totalReal > 0 && !categoriaColapsada(cat.categoriaId)"
            class="ml-3.5 space-y-1"
          >
            <div class="flex items-center gap-2">
              ...
              <span
                class="text-[0.6875rem] shrink-0 font-medium"
                :class="cat.totalReal > cat.total ? 'text-red-400' : 'text-emerald-400'"
              >
            ...
            <div class="flex items-center gap-1">
              ...
              <span
                ...
                :class="cat.totalReal > cat.total ? 'text-red-400' : 'text-emerald-400'"
              >
                {{ cat.totalReal > cat.total ? 'Excede' : 'Ahorra' }} {{ currencySymbol }}&nbsp;{{
                  formatMonto(Math.abs(cat.totalReal - cat.total))
                }}
              </span>
            </div>
          </div>
            ...
            <div
              ...
              :class="
                gasto.estado === 'pagado'
                  ...
                  : esVencido(gasto)
                    ? 'border-l-red-500'
                    : esHoyGasto(gasto) && gasto.estado === 'pendiente'
                      ? 'border-l-orange-500'
                      : 'border-l-orange-400/50'
              "
              :style="{
                transform: `translateX(${swipeOffsets[gasto.id] || 0}px)`,
                transition: swipeDragging[gasto.id] ? 'none' : 'transform 0.25s ease',
              }"
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-xs mt-0.5 flex items-center gap-1.5 flex-wrap">
                      <span
                        v-if="gasto._catNombre"
                        class="inline-flex items-center gap-1 text-theme-text-sec"
                      >
                        <span
                          class="w-1.5 h-1.5 rounded-full inline-block"
                          :style="{ backgroundColor: gasto._catColor }"
                        ></span>
                        {{ gasto._catNombre }}
                      </span>
                      <span
                ...
                <div class="text-right shrink-0">
                  ...
                  <p
                    v-if="gasto.gastoRegistradoFecha"
                    class="mt-1 text-[0.6875rem] text-theme-text-sec"
                  >
              ...
              >
                <button
                  ...
                  @click="
                    esCategoriaAhorro(gasto) ? emit('registrar', gasto) : marcarPagadoRapido(gasto)
                  "
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
...
const ordenLabel = computed(
  () => ordenes.find((o) => o.value === ordenActual.value)?.label || 'Fecha',
)
...
const filtrosConContador = computed(() => {
  ...
  return filtros.map((f) => {
    ...
    if (f.value === 'todos') count = todos.length
      ...
      count = todos.filter((g) => esHoyGasto(g) && g.estado === 'pendiente').length
