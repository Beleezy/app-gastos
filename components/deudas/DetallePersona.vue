<template>
  <div ref="detalleRoot" class="px-4 py-3" data-testid="detalle-persona">
    ...
    <template v-else>
      <!-- Deudas pendientes: extraída a DeudasListaDeudasActivas.vue. Los
           diálogos de confirmación se quedan aquí, que es quien conoce el
           estado de la página. -->
      <DeudasListaDeudasActivas
        v-if="deudasActivasPersona.length > 0"
        :deudas="deudasActivasPersona"
        :tab="tabActual"
        :es-vencida="esVencida"
        @registrar-pago="(d) => emit('registrarPago', d)"
        @editar="(d) => emit('editarDeuda', d)"
        @saldar="confirmarSaldar"
        @eliminar="confirmarEliminarDeuda"
      />
