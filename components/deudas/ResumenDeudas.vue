<template>
  <div class="px-4 lg:px-0 pt-4 lg:pt-6 pb-2">
    <div
      class="relative bg-gradient-to-br from-theme-card to-theme-card/90 rounded-2xl p-4 border border-theme-border overflow-hidden"
    >
      <!-- Decorative accent -->
      <div
      ...
      <div class="relative">
        ...
        <div class="flex items-end justify-between gap-3 flex-wrap">
          <div class="min-w-0">
            ...
            <p class="text-[0.6875rem] text-theme-text-sec mt-1.5">
              {{ countActual }} deuda{{ countActual !== 1 ? 's' : '' }} pendiente{{
                countActual !== 1 ? 's' : ''
              }}
            </p>
          </div>
          ...
          >
            <p
              class="text-[0.6875rem] text-theme-text-sec leading-tight group-hover:text-theme-text-muted transition-colors"
            >
        ...
        <div v-if="tendencia !== null" class="flex items-center gap-1 mt-2">
          ...
          >
            <template v-if="tendencia !== 0">
              {{ tendencia > 0 ? '+' : '-' }}{{ currencySymbol }}&nbsp;{{
                formatMonto(Math.abs(tendencia))
              }}
            </template>
        ...
        >
          ...
          <span class="text-[0.6875rem] font-semibold text-red-400 shrink-0">
            {{ countVencidas }} vencida{{ countVencidas !== 1 ? 's' : '' }}
            {{ tabActual === 'me_deben' ? 'por cobrar' : 'por pagar' }}
          </span>
          <span
            v-if="primeraVencida"
            class="text-[0.6875rem] text-red-400/80 truncate flex-1 min-w-0"
          >
          ...
          <span
            class="ml-auto shrink-0 text-[0.6875rem] font-semibold text-red-400 bg-red-500/15 px-2 py-0.5 rounded-full"
          >
