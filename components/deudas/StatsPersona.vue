<template>
  <div v-if="deudasActivas.length > 1" class="bg-theme-card rounded-2xl p-4 mb-4">
    <p class="text-[0.6875rem] font-medium text-theme-text-sec uppercase tracking-wider mb-2">
      Detalle por deuda
    </p>
        ...
        <div class="flex items-center justify-between mb-1">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <span class="text-[0.6875rem] text-theme-text-sec"
              >{{ currencySymbol }}&nbsp;{{ formatMonto(deuda.montoPendiente) }}</span
            >
            <span
              class="text-[0.6875rem] font-medium w-8 text-right"
              :class="
                porcentajeDeuda(deuda) >= 70
                  ? 'text-emerald-400'
                  : porcentajeDeuda(deuda) > 0
                    ? 'text-theme-accent'
