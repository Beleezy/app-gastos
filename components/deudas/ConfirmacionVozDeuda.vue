    ...
    <template #footer>
      <div class="flex gap-3">
        ...
        <button
          ...
          :disabled="guardando || vozParsing || (deudasParseadas.length === 0 && pagosParseados.length === 0)"
...
const tituloConfirm = computed(() => {
  if (pagosParseados.value.length > 0 && deudasParseadas.value.length === 0) return 'Pagos detectados'
  if (deudasParseadas.value.length > 0 && pagosParseados.value.length > 0) return 'Deudas y pagos detectados'
