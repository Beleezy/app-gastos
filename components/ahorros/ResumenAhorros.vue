<template>
  <div class="px-4 lg:px-0 pt-0 pb-2">
    <div class="relative bg-gradient-to-br from-theme-card to-theme-card/90 rounded-2xl px-4 pt-3 pb-4 space-y-4 border border-theme-border overflow-hidden">
      <div class="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl pointer-events-none"></div>
      ...
      <div class="relative flex items-start justify-between gap-3">
        ...
        <div class="flex flex-col items-end gap-1.5 shrink-0">
          <div class="text-right">
            <p class="text-[0.6875rem] text-theme-text-sec uppercase tracking-wider mb-0.5">Total acumulado</p>
      ...
      <div v-if="metaMensual || metaGlobal" class="space-y-2.5">
        <div v-if="metaMensual">
          <div class="flex items-center justify-between mb-1">
            <span class="text-[0.6875rem] text-theme-text-muted uppercase tracking-wider">Meta mensual</span>
            <span
        ...
        <div v-if="metaGlobal">
          <div class="flex items-center justify-between mb-1">
            <span class="text-[0.6875rem] text-theme-text-muted uppercase tracking-wider">Meta global</span>
            <span
              class="text-[0.6875rem] font-semibold px-1.5 py-0.5 rounded-full"
      ...
      <div v-if="tabs?.length" class="relative flex items-center gap-2 pt-1">
        <button
          ...
          :class="modelValue === tab.value ? 'bg-theme-accent-bg text-theme-accent border-theme-accent' : 'bg-theme-input text-theme-text-sec border-theme-border'"
...
const {
  anioActual, nombreMes, esHoy,
  totalMes, totalGlobal, metaMensual, metaGlobal,
  progresoMensual, progresoGlobal,
