...
- El job `visual` de `e2e.yml` se salta solo mientras no existan baselines en `e2e/visual/__screenshots__/`. Para generarlos, `e2e-visual-baseline.yml` los crea en el runner y abre un PR con los PNG: a mano con `workflow_dispatch`, o **empujando a la rama `ci/generar-baselines-visuales`**. Ese disparador por push existe porque `workflow_dispatch` devuelve 403 a un token de GitHub App sin permiso sobre Actions, y sin él los baselines no se podían arrancar. Los PNG tienen que generarse en el runner: el renderizado de fuentes de una máquina local no coincide y el job fallaría siempre.
