    ...
    <div class="lg:grid lg:grid-cols-[380px_1fr] xl:grid-cols-[420px_1fr] lg:gap-6 lg:mt-4">
      ...
      <div>
        <div class="mb-2 px-4 lg:px-0">
          <div
            class="flex items-center gap-1 rounded-xl border border-theme-border bg-theme-card p-1"
          >
            <button
            ...
            >
              ...
              <span
                ...
                class="text-[0.6875rem] font-bold bg-theme-accent/20 px-1.5 rounded-full"
                >{{ gastosPlaneados.length }}</span
              >
            </button>
            <button
              class="flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium transition-colors"
...
useOverlayBack(showPlantillas, () => {
  showPlantillas.value = false
})
const showPresupuestos = ref(false)
useOverlayBack(showPresupuestos, () => {
  showPresupuestos.value = false
})
...
function exportarPlanificador() {
  ...
  exportarExcel(
    `planificador_${nombreMes.value}_${anioActual.value}`,
    columnas,
