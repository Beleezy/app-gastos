<template>
  <div>
    ...
    <div class="lg:grid lg:grid-cols-[380px_1fr] xl:grid-cols-[420px_1fr] lg:gap-6">
      ...
      <div
        :class="[
          personaSeleccionada ? 'hidden lg:block' : '',
          'lg:sticky lg:top-20 lg:self-start lg:pt-4',
        ]"
      >
        <!-- Vista lista: resumen general "Me deben / Yo debo" -->
        <component :is="DeudasResumenDeudas" v-if="!personaSeleccionada" />
      ...
      <div>
        <Transition name="page" mode="out-in">
          <div :key="personaSeleccionada ? 'detalle' : 'lista'">
            ...
            />
            <div v-else>
              <component :is="DeudasListaPersonas" @seleccionar="onSeleccionarPersona" />
    ...
    <SharedFloatingActionStack
      :visible="!showVozOverlay && !personaSeleccionada"
      :auto-hide="!fabAbierto"
    >
      <Transition name="fab-dial">
        <div v-if="fabAbierto" class="flex flex-col items-end gap-3 self-end">
          ...
          >
            Por voz
            <span
              class="w-10 h-10 rounded-full bg-violet-500 text-white flex items-center justify-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-5 h-5"
          ...
          >
            Nueva deuda
            <span
              class="w-10 h-10 rounded-full bg-theme-accent text-theme-on-accent flex items-center justify-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-5 h-5"
...
<style scoped>
.fab-dial-enter-active {
  transition:
    opacity 0.2s ease-out,
    transform 0.2s ease-out;
}
.fab-dial-leave-active {
  transition:
