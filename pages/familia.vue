<template>
  <div class="min-h-screen pb-4">
    ...
    <div class="px-5 pt-8 pb-3 relative overflow-hidden">
      <div
        class="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-32 bg-fuchsia-500/10 rounded-full blur-3xl"
      ></div>
      <div class="relative flex items-center gap-3 mb-1">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[0.6875rem] text-theme-text-sec mt-0.5">
            Lleva las finanzas de tus familiares
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="px-5 mb-4">
      <div
        class="rounded-2xl border border-dashed border-theme-border bg-theme-card/40 p-3 text-[0.6875rem] text-theme-text-muted leading-relaxed"
      >
        Crea un perfil por cada familiar cuyas finanzas administres (p. ej. papá o abuelo). Al
        <strong class="text-theme-text">Entrar</strong> a un perfil, Registro, Planificador, Ahorro
        y Deudas mostrarán y guardarán los datos de esa persona. Ellos no necesitan cuenta ni
        iniciar sesión.
      </div>
    </div>
    ...
    <div v-if="mostrarForm" class="px-5 mb-4">
      <div class="bg-theme-card border border-theme-border rounded-2xl p-4 space-y-3">
        ...
        <div>
          <label class="block text-[0.6875rem] text-theme-text-muted mb-1">Nombre</label>
          <input
            v-model="form.nombre"
            placeholder="p. ej. Papá"
            maxlength="255"
            class="w-full bg-theme-input border border-theme-border rounded-xl px-3 py-2.5 text-sm"
          />
        </div>
        <div>
          <label class="block text-[0.6875rem] text-theme-text-muted mb-1"
            >Relación <span class="text-theme-text-muted">(opcional)</span></label
          >
          <input
            v-model="form.relacion"
            placeholder="p. ej. Padre, Abuelo, Hijo"
            maxlength="50"
            class="w-full bg-theme-input border border-theme-border rounded-xl px-3 py-2.5 text-sm"
          />
        </div>
        <div>
          <label class="block text-[0.6875rem] text-theme-text-muted mb-1"
            >WhatsApp <span class="text-theme-text-muted">(opcional)</span></label
          >
          <input
            v-model="form.telefono"
            type="tel"
            inputmode="tel"
            placeholder="p. ej. 51987654321"
            maxlength="30"
            class="w-full bg-theme-input border border-theme-border rounded-xl px-3 py-2.5 text-sm"
          />
          <p class="text-[0.6875rem] text-theme-text-muted mt-1">
            Con código de país, sin espacios ni símbolos. Se usa para enviarle reportes.
          </p>
        </div>
        <div>
          <label class="block text-[0.6875rem] text-theme-text-muted mb-1"
            >Presupuesto mensual <span class="text-theme-text-muted">(opcional)</span></label
          >
          <input
        ...
        <div>
          <label class="block text-[0.6875rem] text-theme-text-muted mb-1"
            >Correo de contacto <span class="text-theme-text-muted">(opcional)</span></label
          >
          <input
        ...
        <div>
          <label class="block text-[0.6875rem] text-theme-text-muted mb-1"
            >Fecha de nacimiento <span class="text-theme-text-muted">(opcional)</span></label
          >
          <input
        ...
        <div>
          ...
          <textarea
            ...
            class="w-full bg-theme-input border border-theme-border rounded-xl px-3 py-2 text-sm resize-none"
          ></textarea>
        </div>
        <div class="flex gap-2 pt-1">
          <button
            class="flex-1 px-4 py-2.5 rounded-xl bg-theme-input text-theme-text-sec text-sm"
            @click="cerrarForm"
          >
            Cancelar
          </button>
          <button
            class="flex-1 px-4 py-2.5 rounded-xl bg-fuchsia-500 text-white text-sm font-semibold disabled:opacity-50"
    ...
    <div class="px-5 space-y-2">
      ...
      <div v-else-if="!perfiles.length && !mostrarForm" class="text-center py-10">
        ...
        <p class="text-[0.6875rem] text-theme-text-muted mt-1">
          Crea uno para empezar a administrar sus finanzas
        </p>
      </div>
      ...
      >
        <div class="flex items-center justify-between gap-2">
          <div class="flex items-center gap-2.5 min-w-0">
            <div
              class="w-9 h-9 rounded-lg bg-fuchsia-500/15 flex items-center justify-center text-sm font-bold text-fuchsia-300 shrink-0"
            >
              {{ inicial(p.nombre) }}
            </div>
            <div class="min-w-0">
              <p class="text-sm text-theme-text font-semibold truncate">
                {{ p.nombre
                }}<span
                  v-if="p.relacion"
                  class="text-[0.6875rem] font-normal text-theme-text-muted"
                >
                  · {{ p.relacion }}</span
                >
              </p>
              <p class="text-[0.6875rem] text-theme-text-muted truncate">
                <span v-if="p.presupuesto > 0"
                  >Presup.: {{ currencySymbol }}&nbsp;{{ formatMonto(p.presupuesto) }}</span
                >
          ...
          <div class="flex items-center gap-1 shrink-0">
            <button
              class="px-2.5 py-1.5 rounded-lg bg-fuchsia-500/15 text-fuchsia-400 text-[0.6875rem] font-semibold"
              @click="entrarPerfil(p.id)"
            >
              Entrar
            </button>
            <button
              class="w-11 h-11 rounded-lg text-theme-text-muted hover:text-theme-text hover:bg-theme-border-md"
              aria-label="Editar"
            ...
            <button
              class="w-11 h-11 rounded-lg text-theme-text-muted hover:text-red-400 hover:bg-red-500/10"
              aria-label="Eliminar"
    ...
    <SharedConfirmDialog
      ...
      :message="
        perfilAEliminar
          ? `¿Eliminar el perfil &quot;${perfilAEliminar.nombre}&quot;? Se borrarán también sus gastos, ingresos, planificador, ahorros y deudas.`
          : ''
      "
