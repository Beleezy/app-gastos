<template>
  <div class="min-h-screen pb-4">
    ...
    <div class="px-5 pt-8 pb-3 relative overflow-hidden">
      <div
        class="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-32 bg-sky-500/10 rounded-full blur-3xl"
      ></div>
    ...
    <div class="px-5 space-y-4">
      <!-- Contexto del perfil -->
      <div
        class="rounded-2xl border border-theme-border bg-theme-card p-3 text-xs text-theme-text-sec"
      >
        Generando para: <strong class="text-theme-text">{{ nombrePerfilActivo }}</strong
        >.
        <template v-if="perfiles.length > 0">
          Cambia el perfil en la barra "Viendo:" al inicio de la página si quieres el reporte de
          otro familiar.
        </template>
      </div>
      ...
      <div>
        <label
          class="block text-[0.6875rem] uppercase tracking-wider text-theme-text-muted mb-1.5 px-1"
          >Módulo</label
        >
          ...
          <button
            ...
            :class="
              modulo === m.valor
                ...
                : 'border-theme-border bg-theme-card text-theme-text-sec'
            "
      ...
      <div class="grid grid-cols-2 gap-2">
        <div>
          ...
          <select
            v-model.number="mes"
            class="w-full bg-theme-input border border-theme-border rounded-xl px-3 py-2.5 text-sm text-theme-text"
          >
