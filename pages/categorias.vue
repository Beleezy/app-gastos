<template>
  <div>
    ...
    <div class="max-w-lg lg:max-w-3xl mx-auto lg:mx-0 px-4 lg:px-0 py-4 lg:py-6 space-y-4">
      <p class="text-sm text-theme-text-sec mb-4">
        Personaliza las categorías para clasificar tus gastos. Puedes crear nuevas o elegir de las
        preseleccionadas.
      </p>
      ...
      <ClientOnly>
        <Teleport to="body">
          <Transition name="modal">
            <div
              v-if="mostrarSelectorCategorias"
              class="fixed inset-0 z-50 flex items-end justify-center"
            >
              <div
                class="absolute inset-0 bg-theme-bg/80 backdrop-blur-sm"
                @click="cerrarSelectorCategorias"
              ></div>
              <div
                class="relative w-full max-w-lg bg-theme-card rounded-t-2xl border-t border-theme-border max-h-[85vh] flex flex-col animate-slide-up"
              >
                <!-- Header del modal -->
                <div
                  class="flex items-center justify-between p-4 border-b border-theme-border shrink-0"
                >
                  <h3 class="text-base font-semibold text-theme-text">
                    {{ editandoCategoria ? 'Editar categoria' : 'Agregar categoria' }}
                  </h3>
                  <button
                    class="p-1 rounded-lg hover:bg-theme-border-md transition-colors"
                    @click="cerrarSelectorCategorias"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="w-5 h-5 text-theme-text-sec"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      stroke-width="2"
                ...
                <div class="overflow-y-auto overscroll-contain flex-1 p-4">
                  ...
                  <div v-if="tabCategoria === 'preseleccionadas'">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                      <button
                        ...
                        :class="
                          categoriasExistentes.has(cat.nombre)
                            ? 'border-theme-border bg-theme-border-md opacity-50 cursor-not-allowed'
                            : 'border-theme-border hover:border-theme-accent active:scale-[0.97]'
                        "
                      ...
                      >
                        ...
                        <span
                          class="text-xs text-theme-text line-clamp-2 break-words hyphens-none leading-tight min-w-0 flex-1"
                          >{{ cat.nombre }}</span
                        >
                        <svg
                          v-if="categoriasExistentes.has(cat.nombre)"
                          xmlns="http://www.w3.org/2000/svg"
                          class="w-3.5 h-3.5 text-emerald-400 shrink-0 ml-auto"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          stroke-width="3"
                  ...
                  <div v-if="tabCategoria === 'personalizada'" class="space-y-4">
                        ...
                        <button
                          ...
                          :class="
                            nuevaCategoria.icono === emoji
                              ? 'bg-theme-accent/20 ring-2 ring-theme-accent scale-110'
                              : 'bg-theme-input hover:bg-theme-border-md'
                          "
                    ...
                    <!-- Preview -->
                    <div
                      v-if="nuevaCategoria.nombre && nuevaCategoria.icono"
                    ...
                    >
                      ...
                      <div class="flex items-center gap-2.5">
                        <span
                          ...
                          :style="{ backgroundColor: nuevaCategoria.color + '26' }"
                          >{{ nuevaCategoria.icono }}</span
                        >
        ...
        <Teleport to="body">
          <Transition name="modal">
            ...
            >
              ...
              ></div>
              <div
                class="relative bg-theme-card rounded-2xl border border-theme-border p-5 w-full max-w-sm"
              >
                <div class="text-center">
                  ...
                  <p class="text-sm text-theme-text-sec mb-4">
                    ¿Eliminar "{{ categoriaAEliminar.nombre }}"? No se puede deshacer.
                  </p>
...
useOverlayBack(eliminarAbierto, () => {
